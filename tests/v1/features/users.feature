...
Feature: Users
  ...
  Scenario: Create a user returns "Bad Request" response
    ...
    And body with value {"access_role": "ro", "disabled": false, "email": "test@khulnasoft.com", "handle": "test@khulnasoft.com", "name": "test user"}
  ...
  @replay-only @team:KhulnaSoft/team-aaa-identity
  Scenario: Create a user returns null access role
    Given new "CreateUser" request
    And body with value {"access_role": null, "disabled": false, "email": "test@khulnasoft.com", "handle": "test@khulnasoft.com", "name": "test user"}
    When the request is sent
    Then the response status is 200 User created
