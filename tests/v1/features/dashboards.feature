...
Feature: Dashboards
  ...
  @team:KhulnaSoft/dashboards-backend
  ...
  @generated @skip @team:KhulnaSoft/dashboards-backend
  ...
  Scenario: Create a new dashboard with check_status widget
    ...
    And the response "widgets[0].definition.check" is equal to "khulnasoft.agent.up"
  ...
  Scenario: Create a new dashboard with iframe widget
    ...
    And the response "widgets[0].definition.url" is equal to "https://docs.khulnasoft.com/api/latest/"
  ...
  @skip-validation @team:KhulnaSoft/dashboards-backend
  ...
  Scenario: Create a new dashboard with timeseries widget containing style attributes
    Given new "CreateDashboard" request
    And body with value {"layout_type": "ordered", "title": "{{ unique }} with timeseries widget","widgets": [{"definition": {"type": "timeseries","requests": [{"q": "sum:trace.test.errors{env:prod,service:khulnasoft-api-spec} by {resource_name}.as_count()","on_right_yaxis": false,"style": {"palette": "warm","line_type": "solid","line_width": "normal"},"display_type": "bars"}]}}]}
    When the request is sent
    Then the response status is 200 OK
    ...
    And the response "widgets[0].definition.requests[0].q" is equal to "sum:trace.test.errors{env:prod,service:khulnasoft-api-spec} by {resource_name}.as_count()"
  ...
  @replay-only @team:KhulnaSoft/dashboards-backend
  ...
  @replay-only @skip-validation @team:KhulnaSoft/dashboards-backend @with-pagination
  ...
  Scenario: Revoke shared dashboard invitations returns "Not Found" response
    ...
    And request contains "token" parameter from "REPLACE.ME"
    And body with value {"data": [{"attributes": {"email": "test@khulnasoft.com"}, "type": "public_dashboard_invitation"}]}
    When the request is sent
    Then the response status is 404 Not Found
  ...
  Scenario: Send shared dashboard invitation email returns "OK" response
    ...
    And request contains "token" parameter from "shared_dashboard.token"
    And body with value {"data": {"attributes": {"email": "{{unique_lower_alnum}}@khulnasoft.com"}, "type": "public_dashboard_invitation"}}
    When the request is sent
  ...
  Scenario: Update a shared dashboard returns "Bad Request" response
    ...
    And request contains "token" parameter from "REPLACE.ME"
    And body with value {"global_time": {"live_span": "1h"}, "share_list": ["test@khulnasoft.com", "test2@khulnasoft.com"], "share_type": "invite"}
    When the request is sent
    Then the response status is 400 Bad Request
