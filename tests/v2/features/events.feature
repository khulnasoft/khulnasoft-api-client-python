...
Feature: Events
  ...
  @generated @skip @team:KhulnaSoft/event-management
  Scenario: Get a list of events returns "Bad Request" response
    Given new "ListEvents" request
    When the request is sent
    Then the response status is 400 Bad Request

  @skip-validation @team:KhulnaSoft/event-management
  Scenario: Get a list of events returns "OK" response
    Given new "ListEvents" request
    When the request is sent
    Then the response status is 200 OK

  @replay-only @skip-validation @team:KhulnaSoft/event-management @with-pagination
  ...
  @team:KhulnaSoft/event-management
  ...
  Scenario: Search events returns "OK" response
    Given new "SearchEvents" request
    And body with value {"filter": {"query": "khulnasoft-agent", "from": "2020-09-17T11:48:36+01:00", "to": "2020-09-17T12:48:36+01:00"}, "sort": "timestamp", "page": {"limit": 5}}
    When the request is sent
    Then the response status is 200 OK
    And the response "data" has length 0
