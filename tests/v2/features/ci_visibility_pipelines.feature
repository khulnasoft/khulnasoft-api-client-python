...
Feature: CI Visibility Pipelines
  ...
  your Khulnasoft site over HTTP. See the [CI Pipeline Visibility in Khulnasoft
  page](https://docs.khulnasoft.com/continuous_integration/pipelines/) for
  more information.
  ...
  @generated @skip @team:Khulnasoft/ci-app-backend @team:Khulnasoft/integrations-tools-and-libraries
  ...
  @team:Khulnasoft/ci-app-backend @team:Khulnasoft/integrations-tools-and-libraries
  ...
  @replay-only @skip-validation @team:Khulnasoft/ci-app-backend @team:Khulnasoft/integrations-tools-and-libraries @with-pagination
  ...
  Scenario: Send pipeline event returns "Request accepted for processing" response
    Given new "CreateCIAppPipelineEvent" request
    And body with value {"data": {"attributes": {"resource": {"end": "{{ timeISO('now - 30s') }}", "level": "pipeline", "name": "Deploy to AWS", "partial_retry": false, "start": "{{ timeISO('now - 120s') }}", "status": "success", "unique_id": "3eacb6f3-ff04-4e10-8a9c-46e6d054024a", "url": "https://my-ci-provider.example/pipelines/my-pipeline/run/1","git":{"repository_url":"https://github.com/KhulnaSoft/khulnasoft-agent","sha":"7f263865994b76066c4612fd1965215e7dcb4cd2","author_email":"john.doe@email.com"}}}, "type": "cipipeline_resource_request"}}
    When the request is sent
