...
Feature: Service Definition
  ...
  @generated @skip @team:KhulnaSoft/service-catalog
  ...
  @team:KhulnaSoft/service-catalog
  Scenario: Create or update service definition using schema v2 returns "CREATED" response
    Given new "CreateOrUpdateServiceDefinitions" request
    And body with value {"contacts": [{"contact": "contact@khulnasoft.com", "name": "Team Email", "type": "email"}], "dd-service": "service-{{ unique_lower_alnum }}", "dd-team": "my-team", "docs": [{"name": "Architecture", "provider": "google drive", "url": "https://gdrive/mydoc"}], "extensions": {"myorgextension": "extensionvalue"}, "integrations": {"opsgenie": {"region": "US", "service-url": "https://my-org.opsgenie.com/service/123e4567-e89b-12d3-a456-426614174000"}, "pagerduty": "https://my-org.pagerduty.com/service-directory/PMyService"}, "links": [{"name": "Runbook", "type": "runbook", "url": "https://my-runbook"}], "repos": [{"name": "Source Code", "provider": "GitHub", "url": "https://github.com/KhulnaSoft/schema"}], "schema-version": "v2", "tags": ["my:tag", "service:tag"], "team": "my-team"}
    When the request is sent
    Then the response status is 200 CREATED
  ...
  Scenario: Create or update service definition using schema v2-1 returns "CREATED" response
    Given new "CreateOrUpdateServiceDefinitions" request
    And body with value {"contacts":[{"contact":"contact@khulnasoft.com","name":"Team Email","type":"email"}],"dd-service":"service-{{ unique_lower_alnum }}","extensions":{"myorgextension":"extensionvalue"},"integrations":{"opsgenie":{"region":"US","service-url":"https://my-org.opsgenie.com/service/123e4567-e89b-12d3-a456-426614174000"},"pagerduty":{"service-url":"https://my-org.pagerduty.com/service-directory/PMyService"}},"links":[{"name":"Runbook","type":"runbook","url":"https://my-runbook"},{"name":"Source Code","type":"repo","provider":"GitHub","url":"https://github.com/KhulnaSoft/schema"},{"name":"Architecture","type":"doc","provider":"Gigoogle drivetHub","url":"https://my-runbook"}],"schema-version":"v2.1","tags":["my:tag","service:tag"],"team":"my-team"}
    When the request is sent
    Then the response status is 200 CREATED
  ...
  @replay-only @team:KhulnaSoft/service-catalog
  ...
  @replay-only @skip-validation @team:KhulnaSoft/service-catalog @with-pagination
