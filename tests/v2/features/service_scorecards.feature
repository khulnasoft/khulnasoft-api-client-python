...
Feature: Service Scorecards
  ...
  currently in BETA. If you have any feedback, contact [Khulnasoft
  support](https://docs.khulnasoft.com/help/).
  ...
  Scenario: Create outcomes batch returns "Bad Request" response
    ...
    And body with value {"data": {"attributes": {"results": [{"remarks": "See: <a href=\"https://app.khulnasoft.com/services\">Services</a>", "rule_id": "{{ create_scorecard_rule.data.id }}", "state": "pass", "service_name": ""}]}, "type": "batched-outcome"}}
    When the request is sent
    Then the response status is 400 Bad Request
  ...
  Scenario: Create outcomes batch returns "OK" response
    ...
    And operation "CreateScorecardOutcomesBatch" enabled
    And new "CreateScorecardOutcomesBatch" request
    And body with value {"data": {"attributes": {"results": [{"remarks": "See: <a href=\"https://app.khulnasoft.com/services\">Services</a>", "rule_id": "{{ create_scorecard_rule.data.id }}", "service_name": "my-service", "state": "pass"}]}, "type": "batched-outcome"}}
    When the request is sent
