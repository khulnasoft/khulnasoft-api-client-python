...
Feature: Logs
  Search your logs and send them to your Khulnasoft platform over HTTP.

  Background:
    Given a valid "apiKeyAuth" key in the system
    And an instance of "Logs" API

  @team:KhulnaSoft/logs-app
  ...
  @generated @skip @team:KhulnaSoft/logs-app
  ...
  @replay-only @skip-validation @team:KhulnaSoft/logs-app @with-pagination
  ...
  Scenario: Get a quick list of logs returns "OK" response
    Given a valid "appKeyAuth" key in the system
    And new "ListLogsGet" request
    And request contains "filter[query]" parameter with value "khulnasoft-agent"
  ...
  Scenario: Search logs returns "OK" response
    Given a valid "appKeyAuth" key in the system
    And new "ListLogs" request
    And body with value {"filter": {"query": "khulnasoft-agent", "indexes": ["main"], "from": "2020-09-17T11:48:36+01:00", "to": "2020-09-17T12:48:36+01:00"}, "sort": "timestamp", "page": {"limit": 5}}
    When the request is sent
    Then the response status is 200 OK
    And the response "data" has length 0
  ...
  @integration-only @skip-terraform-config @skip-validation @team:KhulnaSoft/event-platform-intake @team:KhulnaSoft/logs-backend
  ...
  @generated @skip @team:KhulnaSoft/event-platform-intake @team:KhulnaSoft/logs-backend
  ...
  @team:KhulnaSoft/event-platform-intake @team:KhulnaSoft/logs-backend
