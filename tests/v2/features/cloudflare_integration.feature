...
Feature: Cloudflare Integration
  Manage your Khulnasoft Cloudflare integration directly through the Khulnasoft
  API. See the [Cloudflare integration
  page](https://docs.khulnasoft.com/integrations/cloudflare/) for more
  information.
  ...
  @replay-only @team:KhulnaSoft/saas-integrations @team:KhulnaSoft/web-integrations
  ...
  Scenario: Add Cloudflare account returns "CREATED" response
    Given new "CreateCloudflareAccount" request
    And body with value {"data": {"attributes": {"api_key": "fakekey", "email": "dev@khulnasoft.com", "name": "{{ unique_lower_alnum }}"}, "type": "cloudflare-accounts"}}
    When the request is sent
    Then the response status is 201 CREATED
    And the response "data.type" is equal to "cloudflare-accounts"
    And the response "data.attributes.email" is equal to "dev@khulnasoft.com"
  ...
  Scenario: Update Cloudflare account returns "OK" response
    ...
    And new "UpdateCloudflareAccount" request
    And request contains "account_id" parameter from "cloudflare_account.data.id"
    And body with value {"data": {"attributes": {"api_key": "fakekey", "email": "dev@khulnasoft.com", "zones": ["zone-id-3"]}, "type": "cloudflare-accounts"}}
    When the request is sent
    Then the response status is 200 OK
