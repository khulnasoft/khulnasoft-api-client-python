...
Feature: Service Accounts
  ...
  @generated @skip @team:KhulnaSoft/credentials-management @team:KhulnaSoft/team-aaa-identity
  ...
  @team:KhulnaSoft/credentials-management @team:KhulnaSoft/team-aaa-identity
  Scenario: Create a service account returns "OK" response
    Given there is a valid "role" in the system
    And new "CreateServiceAccount" request
    And body with value {"data": {"type": "users", "attributes": {"name": "Test API Client", "email": "{{ unique }}@khulnasoft.com", "service_account": true}, "relationships": {"roles": {"data": [{"id": "{{ role.data.id }}", "type": "roles"}]}}}}
    When the request is sent
